...
function Blueprint({ data }: { data: BlueprintData }) {
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto w-full">
      {/* Idea hero */}
      <Card className="glass bg-gradient-card rounded-2xl md:rounded-3xl p-3 md:p-10 relative overflow-hidden border-gold/30">
        ...
        <div className="relative">
          ...
          <h1 className="mt-1.5 md:mt-5 text-[34px] md:text-[97px] font-bold leading-[1] md:leading-[1.05] tracking-tight">
          ...
          <div className="mt-2.5 md:mt-8 grid grid-cols-2 gap-1.5 md:gap-3">
            {stats.map((s) => (
              <div
                key={s.label}
                className="rounded-lg md:rounded-xl border border-border/50 bg-secondary/30 p-2 md:p-4"
              >
                <div className="flex items-center gap-1 md:gap-2 text-muted-foreground text-[9px] md:text-xs uppercase tracking-wider">
                  <s.icon className="w-2.5 h-2.5 md:w-3.5 md:h-3.5 shrink-0" /> {s.label}
                </div>
                <p className="mt-0.5 md:mt-2 font-semibold text-[11px] md:text-base leading-tight md:leading-snug">{s.value}</p>
              </div>
            ))}
          </div>
        </div>
